...
Tawsel is an Arabic-first food and grocery delivery mobile app prototype.

## Run & Operate

- Use the `Tawsel Expo` workflow to start the Replit preview.
...
The Expo workflow uses Replit-provided runtime domains and `PORT=23658`; no user-provided secret or database is required to preview the current app.
...
- `artifacts/api-server/` — small Express health-check service
- `lib/api-spec/openapi.yaml` — API contract source
- `lib/db/src/schema/` — database schema source

## Architecture decisions

- Tawsel remains an Expo/React Native app rather than being converted to a web-only React app.
...
- Keep the `PORT=23658` assignment when starting Tawsel manually.
- The API server and database packages are present for the workspace but are not needed by the current local mobile preview.
